...
export const parseWithResult = <Schema extends ZodTypeAny>(schema: Schema) =>(
  input: z.input<Schema>
): Result<z.infer<Schema>, ZodError> => {
  const result = schema.safeParse(input);
  return result.success ? ok(result.data) : err(result.error);
};

export const parse = <Schema extends ZodTypeAny>(schema: Schema) =>(
  input: z.input<Schema>
): z.infer<Schema> => {
...
export const normalizeedEventSchema = <T extends ZodRawShape, K extends keyof T>(schema: ZodObject<T>, key: K) => {
